jQuery(document).ready(function ($) {
    ...
    var cards="";
     ...
     var getPosts = $.get('/ayllu/wp-json/wp/v2/posts?per_page=15').done(function (post, status) {
        ...
        $.each(post, function (postKey, postValue) {
            ...
            var medias = $.get('/ayllu/wp-json/wp/v2/media/' + postValue.featured_media).done(function (media, status) {
                ...
                //console.log(img);
                // Construir el cards
                var card = '<div class="option img-fluid' + primero + '" style="--optionBackground:url(' + img + ');"><div class="shadow"></div><div class="label"><div class="icon"><i class="fa-solid fa-laptop-code"></i></div><div class="info"><div class="main">' + postValue.title['rendered'] + '</div><div class="sub">' + postValue.date + '</div></div></div></div>';
