     ...
     $.get('/ayllu/wp-json/wp/v2/posts').done(function (post, status) {
        $.each(post, function (postKey, postValue) {
            ...
            $.get('/ayllu/wp-json/wp/v2/media/' + postValue.featured_media).done(function (media, status) {
                ...
                //console.log(img);
                // Construir el cards
                var cards = '<img src="' + img + '" alt="' + postValue.title['rendered'] + '" class="img-fluid"><div class="shadow"></div><div class="label"><div class="icon"><i class="fa-sharp fa-solid fa-laptop-code"></i></div><div class="info"><h2 class="main text-white">' + postValue.title['rendered'] + '</h2><p class="sub">' + postValue.date + '</p></div></div>';
