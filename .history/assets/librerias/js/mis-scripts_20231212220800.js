jQuery(document).ready(function ($) {
    ...
    var cards="";
     ...
     var getPosts = $.get('/ayllu/wp-json/wp/v2/posts?per_page=7&orderby=date&order=asc').done(function (post, status) {
...
// Botón
    $('#botoncillo').on('click', function(){ // boton que se diagrama en HTML.
        ...
        var boton = $.get('/ayllu/wp-json/wp/v2/posts?per_page=7&orderby=date&order=desc', function(data, status){
            console.log(data);
            ...
            $.each(data, function(key, value){
                ...
                var data = $.get(imagenes, function (datosImagen) {
        ...
        $("#contenedor-tarjetas").on("click", ".option", function () {
   
            $(".option").removeClass("active");
            $(this).addClass("active");
            
            });
        $('#botoncillo').hide('slow');
        $('#relacionadas').hide('slow');
    });
// Botón
