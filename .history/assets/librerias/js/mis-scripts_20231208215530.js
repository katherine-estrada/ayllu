     ...
     $.get('/ayllu/wp-json/wp/v2/posts').done(function (post, status) {
        ...
        $.each(post, function (postKey, postValue) {
            ...
            $.get('/ayllu/wp-json/wp/v2/media/' + postValue.featured_media).done(function (media, status) {
                ...
                //console.log(img);
                // Construir el cards
                var cards = '<div class="option ' + primero + '" style="--optionBackground:url(' + img + ');"><div class="shadow"></div><div class="label"><div class="icon"><i class="fas fa-walking"></i></div><div class="info"><div class="main">' + postValue.title['rendered'] + '</div><div class="sub">' + postValue.date + '</div></div></div></div>';
