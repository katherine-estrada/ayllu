jQuery(document).ready(function ($) {
     ...
     var getPosts = $.get('/ayllu/wp-json/wp/v2/posts?per_page=5').done(function (post, status) {
        ...
        $.each(post, function (postKey, postValue) {
            ...
            var medias = $.get('/ayllu/wp-json/wp/v2/media/' + postValue.featured_media).done(function (media, status) {
                ...
                //console.log(img);
                // Construir el cards
                var card = '<div class="option' + primero + '" data-content-url="${post.link}" style="--optionBackground:url(' + img + ');"><div class="shadow"></div><div class="label"><div class="icon"><i class="fa-solid fa-laptop-code"></i></div><div class="info"><div class="main">' + postValue.title['rendered'] + '</div><div class="sub">' + postValue.date + '</div></div></div></div>';
    ...
    $.ajax({
        ...
        success: function (content) {
            // Actualizar el contenido en el área deseada (puedes elegir un área específica)
