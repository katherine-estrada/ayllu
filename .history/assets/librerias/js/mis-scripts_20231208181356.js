jQuery(document).ready(function ($) {
     ...
     $.get('/ayllu/wp-json/wp/v2/posts').done(function (post, status) {
        $.each(post, function (postKey, postValue) {
                // Construir el cards
                // var cards = '<img src="' + postValue.better_featured_image['source_url'] + '" class="img-fluid option" alt="' + postValue.title['rendered'] + '"><div class="shadow"></div><div class="label"><div class="icon"><i class="fa-sharp fa-solid fa-laptop-code"></i></div><div class="info"><h2 class="main text-white">' + postValue.title['rendered'] + '</h2><p class="sub">' + postValue.date + '</p></div></div></img>';

                var featuredImageUrl = postValue.better_featured_image && postValue.better_featured_image.source_url ? postValue.better_featured_image.source_url : '';
                var cards = '<img src="' + featuredImageUrl + '" class="img-fluid option" alt="' + postValue.title['rendered'] + '"><div class="shadow"></div><div class="label"><div class="icon"><i class="fa-sharp fa-solid fa-laptop-code"></i></div><div class="info"><h2 class="main text-white">' + postValue.title['rendered'] + '</h2><p class="sub">' + postValue.date + '</p></div></div></img>';
